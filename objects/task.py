...
class Task(object):
    ...
    def __init__(self, id, type, ts_submit, submission_site, runtime, resource_amount_requested, parents, workflow_id,
        ...
        self.datatransfers = datatransfer  # list of data transfers (JSON)
        self.events = events  # list of data transfers (JSON)
    ...
    def get_parquet_meta_dict():
        type_info = {
            ...
            "type": np.str,
            "ts_submit": np.int64,
            "submission_site": np.int32,
            "runtime": np.int64,
            "resource_type": np.str,
            "resource_amount_requested": np.float64,
            "parents": np.object,
            "children": np.object,
            "user_id": np.int32,
            "group_id": np.int32,
            ...
            "wait_time": np.int64,
            "params": np.str,
            "memory_requested": np.float64,
            "network_io_time": np.int64,
            "disk_io_time": np.int64,
            "disk_space_requested": np.float64,
            "energy_consumption": np.int64,
            "resource_used": np.int64,
        }
    ...
    def get_parquet_dict(self):
        simple_dict = {
            ...
            "submission_site": np.int32(self.submission_site),
            "runtime": np.int64(self.runtime),
            "resource_type": np.str(self.resource_type),
            "resource_amount_requested": np.float64(self.resource_amount_requested),
            "parents": list(self.parents),
            "children": list(self.children),
            "user_id": np.int32(self.user_id),
            "group_id": np.int32(self.group_id),
            "nfrs": np.str(json.dumps(self.nfrs)),
            "workflow_id": np.int64(self.workflow_id),
            "wait_time": np.int64(self.wait_time),
            "params": np.str(json.dumps(self.params)),
            "memory_requested": np.float64(self.memory_requested),
            "network_io_time": np.int64(self.network_io_time),
            "disk_io_time": np.int64(self.disk_io_time),
            "disk_space_requested": np.float64(self.disk_space_requested),
            "energy_consumption": np.int64(self.energy_consumption),
