...
class Workflow(object):
    ...
    def get_parquet_meta_dict():
        type_info = {
            ...
            "ts_submit": np.int64,
            "task_count": np.int32,
            "critical_path_length": np.int64,
            "critical_path_task_count": np.int32,
            "approx_max_concurrent_tasks": np.int32,
            "nfrs": np.str,
            "scheduler": np.str,
            "total_resources": np.float64,
    ...
    def get_parquet_dict(self):
        simple_dict = {
            ...
            "task_count": np.int32(self.task_count),
            "critical_path_length": np.int64(self.critical_path_length),
            "critical_path_task_count": np.int32(self.critical_path_task_count),
            "approx_max_concurrent_tasks": np.int32(self.max_concurrent_tasks),
            "nfrs": np.str(json.dumps(self.nfrs)),
            "scheduler": np.str(self.scheduler),
            "total_resources": np.float64(self.total_resources),
            "total_memory_usage": np.float64(self.total_memory_usage),
            "total_network_usage": np.float64(self.total_network_usage),
            "total_disk_space_usage": np.float64(self.total_disk_space_usage),
            "total_energy_consumption": np.int64(self.total_energy_consumption),
        }
