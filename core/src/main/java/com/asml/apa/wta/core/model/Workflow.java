...
public class Workflow implements BaseTraceObject {
  ...
  private static Schema workflowSchema = SchemaBuilder.record("workflow")
          .namespace("com.asml.apa.wta.core.model").fields()
          .name("id").type().longType().noDefault()
          .name("submitTime").type().longType().noDefault()
          .name("tasks").type().nullable().array().items().longType().noDefault()
          .name("numberOfTasks").type().intType().noDefault()
          .name("criticalPathLength").type().intType().noDefault()
          .name("criticalPathTaskCount").type().intType().noDefault()
          .name("maxNumberOfCocurrentTasks").type().intType().noDefault()
          .name("nfrs").type().nullable().stringType().noDefault()
          .name("scheduler").type().nullable().stringType().noDefault()
          .name("domain").type().nullable().stringType().noDefault()
          .name("applicationName").type().nullable().stringType().noDefault()
          .name("applicationField").type().nullable().stringType().noDefault()
          .name("totalResources").type().doubleType().noDefault()
          .name("totalMemoryUsage").type().doubleType().noDefault()
          .name("totalNetworkUsage").type().longType().noDefault()
          .name("totalDiskSpaceUsage").type().longType().noDefault()
          .name("totalEnergyConsumption").type().longType().noDefault()
          .endRecord();
  ...
  public static GenericRecord convertWorkflowToRecord(Workflow workflow) {
    ...
    record.put("tasks", (Long[]) Arrays.stream(workflow.tasks).map(x -> x.getId()).toArray());
